...
import { SiAdobe, SiAngular, SiCanva, SiCss3, SiFigma, SiGithub, SiHtml5, SiJavascript, SiJira,  SiMongodb, SiMysql, SiNextdotjs, SiNodedotjs, SiPostman, SiPython, SiReact, SiSpringboot, SiTrello, SiTypescript } from "react-icons/si";
import { FaFileExcel, FaFilePowerpoint, FaJava, FaMicrosoft } from "react-icons/fa6";
...
export const data = [
    {
        ...
        tools:[
            ...
            { id: "8F", skill: "Libreries: Material UI , Tailwind ,Bootstrap", icon: null },
        ]
    },
    {
        area:"UX/UI",
...
export const Skills = () => {
    ...
    const [dataSoft,setDatasoft] = useState(["Effective communication", "Teamwork", "Problem solving",
         "Adaptability", "Responsibility", "Autonomy", "Critical thinking",]);
    ...
    return(
        <div id="skills" className="mx-2 m-auto bg-cardB p-2">
            ...
            <div className="bg-background grid md:grid-cols-3 grid-flow-rows gap-2 p-2 shadow-lg">
                 {dataHard.map(ele => <><ul className="uppercase border p-2">{ele.area}:
                    {ele.tools.map(ele => <p className="flex gap-2">{ele.icon ? <ele.icon /> : "◾" }<li>{ele.skill}</li></p>  )}
                    </ul>
                 </> 
                )}       
            </div>
